...
export default function MinutesSigning() {
  ...
  const [justSigned, setJustSigned] = useState<{ hash: string; signedAt: string } | null>(null);
  ...
  const sigHash = justSigned?.hash || myExistingSig?.signatureHash;
              ...
              {m.signatures.map((sig: any) => (
                <div key={sig.id} className="flex items-start gap-3" data-testid={`existing-sig-${sig.id}`}>
                  ...
                  <div>
                    ...
                    <div className="text-xs text-[#86868b] font-mono mt-0.5">Hash: {sig.signatureHash?.slice(0, 20)}...</div>
          ...
          {hasSigned ? (
            <div className="space-y-3" data-testid="signature-block">
              ...
              <div className="text-xs text-[#86868b] font-mono">
                Hash: {sigHash?.slice(0, 32)}...
              </div>
              <div className="text-xs text-[#34c759] flex items-center gap-1">
                <CheckCircle size={12} /> SHA-256 signed
              </div>
            </div>
          ) : (
            <div className="space-y-4">
              <div>
                ...
                <div className="text-xs text-[#86868b] mt-0.5">By signing, you confirm these minutes are accurate. A SHA-256 hash will be recorded.</div>
              </div>
              <div
              ...
              <button
                onClick={handleSign}
                disabled={signMinutes.isPending}
